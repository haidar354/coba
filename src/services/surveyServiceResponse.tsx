...
export const surveyService = {
  ...
  getAllSurveys: async (params = {}) => {
    ...
    if (search.trim().length > 2) {
      ...
      response = await api.get("/api/academic/responses", {
        params: queryParams,
      });
  ...
  getSurveyById: async (id) => {
    const response = await api.get(`/api/academic/responses/${id}`);
    return response;
  },

  // Create new survey
  createSurvey: async (surveyData) => {
    const response = await api.post("/api/academic/responses", surveyData);
    return response;
  },

  // Update survey
  updateSurvey: async (id, surveyData) => {
    const response = await api.put(`/api/academic/responses/${id}`, surveyData);
    return response;
  },

  // Delete survey
  deleteSurvey: async (id) => {
    const response = await api.delete(`/api/academic/responses/${id}`);
    return response;
  },
  ...
  uploadSurveysXLSX: async (file) => {
    ...
    const response = await api.post("/api/academic/responses/upload", formData, {
      headers: {
  ...
  downloadSurveysXLSX: async (params = {}) => {
    ...
    const response = await api.get("/api/academic/responses/download/xlsx", {
