// StatisticsCardCustom.tsx (Made responsive: adjusted fonts, icons for sm/md/lg)
import {
  CheckCircle,
  FileText,
  LogOut,
...
export const StatisticsCard = ({ type, count, label, text }) => {
  ...
  const linkCustom = () => {
    switch (type) {
      ...
      case "responsive":
        return "/tu/guest-visits/landing/responsive";
    }
  };
  ...
  const getIcon = () => {
    switch (type) {
      ...
      case "sekolah":
        return <LogOut className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12" />;
      case "responsive":
        return <LogOut className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12" />;
      default:
        return (
  ...
  return (
    ...
    >
      <a href={linkCustom()}>
        ...
        <div className="absolute bottom-0 right-0 w-32 h-32 sm:w-36 sm:h-36 md:w-40 md:h-40 opacity-10"></div>
      </a>
    </div>
  );
};
